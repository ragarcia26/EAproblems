	...
	public static void searchOne(String input){
		try{
			...
			while ((log = brFile.readLine()) != null) {
				...
				String actualRequest=log.substring(NthIndexOf(log,"\"",3)+1,NthIndexOf(log,"\"",4));
				...
				if(input.contains("-e")){
					if(exclusiveCheck(os,actualBrowser) && exclusiveCheck(browser,actualBrowser) &&
							...
							exclusiveCheck(file,actualFile) && exclusiveCheck(requester,actualRequest)){
					...
					if((actualBrowser.contains(os) && !os.equals("")) || (actualBrowser.contains(browser) && !browser.equals("")) ||
							...
							(actualFile.contains(file) && !file.equals("")) || (actualRequest.contains(requester) && !requester.equals(""))){
						System.out.println(log);
						outcomes++;
					}
				}
